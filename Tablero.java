...
public class Tablero {
    ...
    public String toString() {
        ...
        for (fila=NFILAS-1; fila >= 0; fila--) {
            ...
            for (col=0; col < NCOLUMNAS; col++) {
                result.append((MARCAS[_casillas[col][fila]]+"|"));
            }
            result.append("\n");
        }
    ...
    */
    ...
    public int getCasilla(int columna, int fila) {
        if (columna >= 0 && columna < NCOLUMNAS && fila >= 0 && fila < NFILAS) {
            return _casillas[columna][fila];
        } else {
            // Devuelve un valor fuera de rango si la casilla está fuera del tablero
            return -1;
        }
    }

    public void quitarFicha(int columna) {
        if (columna >= 0 && columna < NCOLUMNAS && _posicionLibre[columna] > 0) {
            _casillas[columna][_posicionLibre[columna] - 1] = VACIO;
            _posicionLibre[columna]--;
        }
    }

    public boolean esCasillaValida(int columna, int fila) {
        return columna >= 0 && columna < NCOLUMNAS && fila >= 0 && fila < NFILAS;
    }
